...
export class AuthService {
  ...
  login(username: string, password: string): Observable<AccessData> {
    return this.http.post<AccessData>(`${this.hostUrl}/authenticate`, {
      grant_type: 'password',
      username,
      password,
    });
  ...
  refreshToken(): Observable<AccessData> {
    ...
    return this.http.post<AccessData>(`${this.hostUrl}/refresh-token`, {
  ...
  getAuthUser(): Observable<AuthUser> {
    return this.http.get<AuthUser>(`${this.hostUrl}`);
  }
}
